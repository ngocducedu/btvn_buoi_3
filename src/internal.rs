...
impl Contract {
    pub(crate) fn internal_add_token_to_owner(&mut self, token_id: &TokenId, account_id: &AccountId) {
        // Neu account_id da co ds token roi, thi lay ds token dang co
        // Neu account_id chua co trong tokens_per_owner thi tao moi tokens_Set
        let mut tokens_set = self.tokens_per_owner.get(account_id).unwrap_or_else(|| {
            UnorderedSet::new(StorageKey::TokenPerOwnerInnerKey { account_id_hash: hash_account_id(account_id) }.try_to_vec().unwrap())
        });
        ...
        if sender_id != &token.owner_id {
            ...
            if let Some(enforced_approval_id) = approval_id {
                ...
                assert_eq!(
                    ...
                    "The actual approval id {} is different from the given approbal id {}",
                    actual_approval_id, enforced_approval_id
                )
            }
        };
