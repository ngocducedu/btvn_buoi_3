...
impl  Contract {
    /*
        - Yeu cau use nap tien de cover phi luu tru
        - Them token vao  token_by_id
        - Them token vao danh sach so huu boi owner
        - Refund lai Near cho user deposit thua
    */
    ...
    pub fn nft_mint(&mut self, token_id: TokenId, metadata: TokenMetadata, receiver_id: AccountId, perpetual_royalties: Option<HashMap<AccountId, u32>>) {
        ...
        if let Some(perpetual_royalties) = perpetual_royalties {
            //make sure that the lenghth of the perpetual royalties is below 7 since we wont have enough GAS to pay out that many people
